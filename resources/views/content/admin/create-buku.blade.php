...
<body class="bg-gray-100">
    <div class="container mx-auto p-4">
        ...
        <form action="{{ route('bukus.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('POST')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="isbn" id="isbn" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}-[0-9]{2}-[0-9]{1}" maxlength="17" 
                    placeholder="978-XXX-XXXX-XX-X" value="{{ old('isbn') }}" required 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                <p class="text-gray-600 text-xs italic">Masukkan 13 digit ISBN dengan format yang benar.</p>
                @error('isbn')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="tahun_terbit" id="tahun_terbit" 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    @for ($tahun = 1901; $tahun <= date('Y'); $tahun++)
            ...
            <div class="mb-4">
                ...
                <input type="file" name="cover_buku" id="cover_buku" required 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                @error('cover_buku')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4" required 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="stok_buku" id="stok_buku" value="20" readonly 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            ...
            <div class="flex items-center gap-x-5">
                ...
                <a href="{{ route('bukus.buku') }}" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                    Batal
                </a>
            </div>
        </form>
