<x-layout>
    <section>
        {{-- search bar member --}}
        <form action="{{ route('admin.members.search') }}" method="GET" class="max-w-md mx-auto xl:ml-0 mt-10">
        ...
        {{-- card member --}}
        <div class="mt-20 flex items-center gap-7 flex-wrap justify-center xl:justify-start">
            @if ($members->isEmpty())
                <p class="text-gray-500 text-center mt-5">Member tidak ditemukan.</p>
            @else
                @foreach ($members as $member)
                    <x-pages.admin.card-member :nama="$member->name" :email="$member->email" :role="$member->role" />
                @endforeach
            @endif
        </div>
